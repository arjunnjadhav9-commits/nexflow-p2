-- Fixes confirm_bom_issue's stock check: the v1 loop checked each BOM line
-- independently against the full balance, so the same material appearing on
-- two BOM lines (e.g. FLG-001 consumed by two different steps) could pass
-- both checks while their combined consumption overdraws stock. Aggregate
-- required qty per material_id first, then check each unique material once
-- against its total.
